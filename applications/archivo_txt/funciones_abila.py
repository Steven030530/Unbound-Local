...
class Ingresos_Abila:
    ...
    def aporte_navidad(colectivo,fecha,consecutivo,path):
        ...
        try:
            ...
            abila_ingreso["DATE"] = fecha
            abila_ingreso["DOCUMENT"] = "RE000" + consecutivo
            abila_ingreso["DESCRIPTION DOC"] = "APORTE NAVIDAD DEL MES DE " + dict_calendar.get(fecha[0:2])
            abila_ingreso["DATE2"] = fecha
            abila_ingreso["SUBP"] = "COAN-" + data["SUBP"]
            abila_ingreso["FOUND"] = "25"
            abila_ingreso["GL"] = "20110"
            abila_ingreso["DPTO"] = "0"
            abila_ingreso["CH"] = "CH" + data["CH"].astype(str)
            abila_ingreso["DEBIT"] = ""
            abila_ingreso["CREDIT"] = data["Navidad"]
            abila_ingreso.drop(columns="XXXXXXXX",inplace=True)
            abila_ingreso.drop(abila_ingreso.loc[abila_ingreso['CREDIT']==0].index, inplace=True) 
            abila_ingreso.reset_index(drop=True,inplace=True)
            ...
            abila_ingreso.to_excel("INGRESO NAVIDAD " + sheet + ".xlsx",index=False)
...
class Egreso_Abila:
    ...
    def egreso_navidad(colectivo,empresa,fecha,consecutivo,path):
            ...
            try:
                # Creamos el archivo para importar en abila
                    ...
                    abila_egreso["DATE"] = fecha
                    abila_egreso["DOCUMENT"] = consecutivo
                    abila_egreso["DESCRIPTION DOC"] = "NAVIDAD MES DE " + dict_calendar.get(fecha[0:2])
                    abila_egreso["DATE2"] = fecha
                    abila_egreso["EMPTY"] = ""
                    abila_egreso["SUBP"] = "COAN-" + data["SUBP"]
                    abila_egreso["FOUND"] = "25"
                    abila_egreso["GL"] = "20110"
                    abila_egreso["DPTO"] = "0"
                    abila_egreso["CH"] = "CH" + data["CH"].astype(str)
                    abila_egreso["DEBIT"] = data["Navidad"]
                    abila_egreso["CREDIT"] = ""
                    abila_egreso.drop(columns="XXXXXXXX",inplace=True)
                    abila_egreso.drop(abila_egreso.loc[abila_egreso['CREDIT']==0].index, inplace=True) 
                    abila_egreso.reset_index(drop=True,inplace=True)
                    ...
                    abila_egreso.to_excel(r"C:\Users\darwi\Desktop\proyectounbound - Local\DISPERSION NAVIDAD " + empresa + " " + sheet + ".xlsx",index=False)
